...
pub fn watch(event_tx: tokio_mpsc::Sender<DaemonEvent>) {
    ...
    thread::spawn(move || {
        ...
        if let Err(e) = watcher.watch(&config_dir, RecursiveMode::NonRecursive) {
            log::warn!("Failed to watch config directory '{}': {e}", config_dir.display());
            return;
        }
        log::info!("Watching config directory '{}' for changes", config_dir.display());
        ...
        for event in rx {
            match event {
                Ok(event) if matches!(event.kind, EventKind::Modify(_)) => {
                    ...
                    let _ = event_tx.try_send(DaemonEvent::Command(
