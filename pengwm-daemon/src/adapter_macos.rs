...
impl OsAdapter for MacOsAdapter {
    ...
    fn poll_windows_for_pid(&mut self, pid: i32) -> Vec<WindowId> {
        ...
        for (element, window_id) in windows {
            ...
            self.ctx.cache_mut().insert(window_id, (element, pid));
    ...
    fn set_window_rect(
        ...
        let element = self.cache_get_element(window_id)
            .map(|(e, _)| e)
            .ok_or_else(|| {
                anyhow::anyhow!("element not found in cache for window {}", window_id)
            })?;
        unsafe { ax_element::set_window_rect(element, rect) }
    }

    fn close_window(&mut self, window_id: WindowId) {
        if let Some((element, _)) = self.cache_get_element(window_id) {
