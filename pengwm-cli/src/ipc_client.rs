...
pub fn send_command(cmd: &Command) -> Result<String, String> {
    ...
    let mut stream = UnixStream::connect(SOCKET_PATH)
        .map_err(|e| format!("could not connect to daemon at {SOCKET_PATH}: {e}\n       is pengwm-daemon running?"))?;

    stream.write_all(body.as_bytes()).map_err(|e| format!("write error: {e}"))?;
    stream.shutdown(std::net::Shutdown::Write).map_err(|e| format!("shutdown error: {e}"))?;
